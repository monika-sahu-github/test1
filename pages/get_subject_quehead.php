...
//echo "SELECT subque.id as id, subque.quehead as quehead, count(*) as ctt FROM subject_quehead as subque INNER JOIN exam_question_tbl as examtbl ON subque.id=examtbl.sub_subjecthead_id and subque.main_subject_id='$question_head_id' GROUP by examtbl.sub_subjecthead_id ORDER BY ctt DESC";
                       ...
                       if($selExam->rowCount() > 0)
                        {      //echo "??".$selExam->rowCount();?>
                           ...
                           <?php while ($selExamRow = $selExam->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            $topic_id = $selExamRow['id'];
                            //echo "select * from exam_question_tbl where sub_topic_id!=0 and sub_topic_id in (select id from sub_topic_master where sub_subjecthead_id = '$topic_id')";
                            $get_sub_topic_query = $conn->query("select * from exam_question_tbl where sub_topic_id!=0 and sub_topic_id in (select id from sub_topic_master where sub_subjecthead_id = '$topic_id')");
                    ?>
                    <div class="col-md-3 col-xl-4">
                        <?php 
                          if($get_sub_topic_query->rowCount() > 0)
                        {   ?>
                    <a href="home.php?page=select_subtopic&topic_id=<?php echo openssl_encrypt($topic_id, $ciphering, 
$key, $options, $iv); ?>&exam_type=<?php echo $_REQUEST['exam_type']; ?>&source=<?php echo $val1; ?>&question_head_id=<?php echo $_REQUEST['question_head_id']; ?>&queheadid=<?php echo openssl_encrypt($selExamRow['id'], $ciphering, 
