...
///////////////For question answer submitted///////////////
  ...
  $num3 = 1;
  $easyy = $_REQUEST["easy"];
  $mediumm = $_REQUEST["medium"];
  $hardd = $_REQUEST["hard"];
  $easyy2 = $_REQUEST["easy2"];
  $mediumm2 = $_REQUEST["medium2"];
  $hardd2 = $_REQUEST["hard2"];
...
$sql2 = "select * from exam_question_tbl where sub_subjecthead_id='$queheadid' limit $_SESSION[cntque] ";
 ...
 if($selected!='selected'){
 if($type=='previous'){
// if($easyy>$easyy2 && $easyy!=$easyy2){
//     $difficulty = " and dificulty_level = 1";
//     $easy2 = $easyy2-1;
// } else if($mediumm>$mediumm2){
//     $difficulty = " and dificulty_level = 2";
//     $medium2 = $mediumm2-1;
// } else if($hardd>$hardd2 && $cntque2!=1){
//     $difficulty = " and dificulty_level = 3";
//     $hard2 = $hardd2-1;
// } else {
//     // $difficulty = " and dificulty_level = 1";
//     // $easy2 = $easyy2+1;
...
////////////////////////////////////////////////////////////////////////////////////////////
  ...
  $crt = "  order by eqt_id asc LIMIT 1";
...
///////////////For select new question answer select///////////////
    ...
    $sql = "select * from exam_question_tbl where sub_subjecthead_id='$queheadid' and exam_status = 'active' $difficulty $maxcrt $crt   ";
     // $sql_easy = "(SELECT * FROM `exam_question_tbl` WHERE `dificulty_level` = 1 AND `sub_subjecthead_id` = '$queheadid' LIMIT $easy)";
     // $sql_medium = "(SELECT * FROM `exam_question_tbl` WHERE `dificulty_level` = 2 AND `sub_subjecthead_id` = '$queheadid' LIMIT $medium)";
     // $sql_hard = "(SELECT * FROM `exam_question_tbl` WHERE `dificulty_level` = 3 AND `sub_subjecthead_id` = '$queheadid' LIMIT $hard)";
     // $sql  = $sql_easy.' UNION '.$sql_easy.' UNION '.$sql_easy;
    $result = $conn->query($sql);
    $num = $result->rowCount();
}
...
if(($exam_comp=='done' && $type=='save_and_next') || ($exam_comp=='done' && $type=='skip')){
  //$num = 0;
    ...
    //$question = $sql;
 ?> 
<b><?php echo $num3;  ?>. </b>|||<p><b><?php echo  $sql."<br>".$question; ?></b></p>
                        <?php $numm = 1; 
...
|||<?php echo $eqt_id."|||".$num2."|||".$num3."|||".$for_previous_id."|||".$num3;  } else { echo "not found |||".$queheadid."|||".$num2."|||".$num3;
    ...
    margin-left: 40px;" >
                ...
                <div class="swatch-holder bg-warning switch-header-cs-class qtip tip-top" data-tip="Mark For Review" data-class="bg-primary header-text-light">
                                        ...
                                        <div id="nxt" class="col-sm-4 sug" id="na">
    ...
    <div class="box" style="width: 100%;">
        <div class="box-innerss" id="fflgdiv" style="">
          <div class="row" id="allq">
            <?php 
            $maxcrt = "";
     if($max_que_id!=$match_maxid && $match_maxid!=""){
      $maxcrt = "and eqt_id > '$match_maxid'";
     }
...
//$flag_que = $conn->query("select * from exam_question_tbl where sub_subjecthead_id='$queheadid' and exam_status = 'active' $maxcrt ORDER by eqt_id asc limit $_SESSION[cntque]");
     $sql_easy = "(SELECT * FROM `exam_question_tbl` WHERE `dificulty_level` = 1 AND `sub_subjecthead_id` = '$queheadid' $maxcrt LIMIT $easy)";
     $sql_medium = "(SELECT * FROM `exam_question_tbl` WHERE `dificulty_level` = 2 AND `sub_subjecthead_id` = '$queheadid' $maxcrt LIMIT $medium)";
     $sql_hard = "(SELECT * FROM `exam_question_tbl` WHERE `dificulty_level` = 3 AND `sub_subjecthead_id` = '$queheadid' $maxcrt LIMIT $hard)";
      echo $sqlf  = $sql_easy.' UNION '.$sql_medium.' UNION '.$sql_hard;
...
<span style="text-decoration: none; cursor:pointer; " <?php //if($status==0){ ?>onclick="get_question(<?php echo $i ?>,<?php echo $queheadidd; ?>,'selected','');" <?php //} ?>><div class="swatch-holder <?php echo $color; ?> switch-header-cs-class "  title='<?php echo $quehead; ?>' data-class="bg-primary header-text-light" <?php echo $qqid; ?> >
    ...
    </div><?php } echo "|||".$implode."|||".$easy."|||".$medium."|||".$hard."|||".$easy2."|||".$medium2."|||".$easy2;
    //} 
